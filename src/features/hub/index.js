...
function Hub() {
  ...
  const fetchHub = async (filterValue, textValue) => {
    try {
      const response = await fetch(
        `https://api.logisticacastrofallas.com/api/Panama?numFilter=${filterValue}&textFilter=${textValue}&type=2`
      );
      const data = await response.json();
      if (data.isSuccess) {
        const hubData = data.data.value;
      ...
      dispatch(
        showNotification({ message: "Error fetching hub", type: "error" })
      );
    }
  };

  useEffect(() => {
    fetchHub(filter, textFilter);
  }, [filter, textFilter]);
  ...
  const handleDownloadExcel = async () => {
    try {
      const response = await axios.get(
        "https://api.logisticacastrofallas.com/api/Panama/Download?numFilter=0&type=2",
        {
      ...
      link.href = url;
      link.setAttribute("download", "reporte_hub.xlsx"); // Nombre del archivo
  ...
  return (
    <>
      <TitleCard title="Reporte Cargas Hub Panamá" topMargin="mt-2">
        <div className="mb-4">
