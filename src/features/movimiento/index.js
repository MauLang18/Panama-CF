...
function Movimiento() {
  ...
  const fetchMovimiento = async (filterValue, textValue) => {
    try {
      const response = await fetch(
        `https://api.logisticacastrofallas.com/api/Panama?numFilter=${filterValue}&textFilter=${textValue}&type=3`
      );
      const data = await response.json();
      if (data.isSuccess) {
        const movimientoData = data.data.value;
      ...
      dispatch(
        showNotification({
          message: "Error fetching movimiento",
          type: "error",
        })
      );
    }
  ...
  useEffect(() => {
    fetchMovimiento(filter, textFilter);
  }, [filter, textFilter]);
  ...
  const handleDownloadExcel = async () => {
    try {
      const response = await axios.get(
        "https://api.logisticacastrofallas.com/api/Panama/Download?numFilter=0&type=3",
        {
      ...
      link.href = url;
      link.setAttribute("download", "reporte_movimiento.xlsx"); // Nombre del archivo
  ...
  return (
    <>
      <TitleCard title="Reporte Movimiento Panamá-CRC" topMargin="mt-2">
        <div className="mb-4">
